...
void UserManualWidget::LoadDefaultContent() {
    browser_->setHtml(QStringLiteral(
        ...
        "<li><b>Include transferable shader cache</b> carries the shaders suyu has already built, which reduces stutter the first time you play the package.</li>"
        "<li>The progress bar and status line follow the real work, for example <i>Compiling 312/940 files (main)</i>.</li>"
        "</ul>"
        ...
        "<li><b>A JIT package stutters:</b> play the game in suyu first, then re-export with <b>Include transferable shader cache</b>. The title bar shows <i>Building N shaders</i> when a stutter comes from a new shader.</li>"
