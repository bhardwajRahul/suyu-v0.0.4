...
void UserManualWidget::LoadDefaultContent() {
    browser_->setHtml(QStringLiteral(
        ...
        "<p>Use the Steam shortcut action from a game entry to add or update a shortcut that launches the selected game through suyu. The Export Game dialog does not currently add standalone exports to Steam; add its generated launcher manually if needed.</p>"
        ...
        "</ul>"
        "<h2>Export Game</h2>"
        "<p>Use <b>File → Export Game</b> to choose a title, target platform, CPU backend, and Source or Build output. Windows Build attempts to compile a standalone launcher; Linux and macOS currently produce source artifacts without a bundled runtime. Dynarmic baseline exports are Windows-only. The Steam option in this dialog is unavailable for standalone exports.</p>"
        "<p>Include save data, shader cache, or custom configuration only when you intend to copy those files into the export. Large titles can require substantial space and build time; inspect the resolved update and RomFS before relying on a package.</p>"
